...
class TasksViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, ScannerDelegate,AVCaptureMetadataOutputObjectsDelegate {
    ...
    // Mark - Scanner delegate methods
        ...
        func scanCompleted(withCode code: String)
        {
            ...
            // pass the scanned bar code to the CreateIssueViewController and Query MongoDB Real
            let queryStr: String =  "assetId == '"+code+"'";
            print(queryStr);
            print("issues that contain assetIDs: \(issues.filter(queryStr).count)");
            if(issues.filter(queryStr).count > 0 ){
                scanner?.requestCaptureSessionStopRunning()
            self.navigationController!.pushViewController(CreateIssueViewController(_assets: self.project, _assetRealm: self.realm, _code: currentBarcode!), animated: true);
            }else{
                self.showToast(message: "Asset not found...", seconds: 0.6)
            }
    ...
    required init(project: Asset?, assetRealm: Realm) {
