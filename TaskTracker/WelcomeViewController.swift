        ...
        app.login(withCredential: AppCredentials(username: username!, password: password!)) { [weak self](maybeUser, error) in
            ...
            DispatchQueue.main.sync {
                ...
                if (!USE_PROJECTS_PAGE) {
                    ...
                    let partitionValue = "Common"

                    // Open a realm.
                    let assetRealm = try! Realm(configuration: user.configuration(partitionValue: partitionValue))

                    self!.navigationController!.pushViewController(TasksViewController(project: nil, assetRealm: assetRealm), animated: true);
